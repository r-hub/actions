...
async function install_macos(versions) {
    ...
    var script = path.join(__dirname, "/installer.sh");
    if (process.pkg) {
        console.log(__dirname);
	console.log(__filename);
	console.log(process.cwd());
//        child_process.execSync("ls -l /snapshot/node-install-rstats/lib/install-macos.js", { stdio: "inherit" })
//        child_process.execSync("pwd; ls", { stdio: "inherit" })
        script = path.join(process.cwd(), "lib/installer.sh");
    }
    ...
    if (sudo_test.failed) {
        ...
        try {
            await sudo(
                script + ' ' + script + ' ' + filenames.join(" ") + " 2>&1 >> " + outfile,
                { name: 'installrstats' }
            )
        } catch(error) {
            tail.unwatch()
